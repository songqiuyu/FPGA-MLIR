...
验证策略：
  1. 用 coa.mlir_parser 从 MLIR 重新生成参考 VLIW
...
def generate_reference_vliw(mlir_path: str) -> List[bytes]:
    """
    Re-generate reference VLIW from lowered COA MLIR using coa.mlir_parser.
    Falls back to an empty list if the file cannot be parsed.
    """
    try:
        layers = parse_all_layers(mlir_path, verbose=False)
    except Exception as e:
        print(f"[Verify] WARNING: parse_all_layers failed: {e}")
        return []
    ref_instructions = []
    for layer in layers:
        try:
...
def main():
    ...
    if not os.path.exists(VLIW_FILE):
        ...
        print("  Run bash compile.sh first.")
        sys.exit(1)
